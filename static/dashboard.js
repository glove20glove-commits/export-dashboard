...
function updateTable(data) {
    ...
    const mobile = document.getElementById('company-mobile-cards');
    tbody.innerHTML = '';
    data.forEach(d => {
            ...
            mobile.innerHTML = data.map(d => `
                <article class="mobile-card">
                    ...
                    <div class="m-grid">
                        ...
                        <div class="m-k">수입액(천불)</div><div class="m-v">${num(d.import_amt)}</div>
                        <div class="m-k">수입증감률(%)</div><div class="m-v ${d.import_rate < 0 ? 'negative' : 'positive'}">${Number(d.import_rate || 0).toFixed(1)}</div>
                        <div class="m-k">무역수지(천불)</div><div class="m-v">${num(d.balance)}</div>
                    </div>
                </article>
            `).join('');
        }
